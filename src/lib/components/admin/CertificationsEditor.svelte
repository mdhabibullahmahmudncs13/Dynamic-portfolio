...
<div class="space-y-6">
  <div class="flex justify-between items-center">
    <h2 class="text-3xl font-bold text-cyber-blue">Certifications Management</h2>
    ...
    <div class="card">
      ...
      <form on:submit={handleSubmit} class="space-y-4">
        ...
        <div class="grid md:grid-cols-2 gap-4">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-300 mb-2">Credential URL</label>
            <input type="url" bind:value={formData.credentialUrl} class="input-field" />
          </div>
        </div>

        <div>
          <label class="block text-sm font-medium text-gray-300 mb-2">Badge/Certificate Image</label>
          <input type="file" accept="image/*" on:change={handleImageUpload} class="input-field" />
          {#if formData.imageUrl}
            <img src={formData.imageUrl} alt="Preview" class="mt-2 w-48 h-auto rounded-lg" />
          {/if}
        </div>
      ...
      {#each certifications as cert}
        <div class="card">
          <div class="flex gap-4">
            {#if cert.imageUrl}
              <img src={cert.imageUrl} alt={cert.name} class="w-24 h-24 object-cover rounded-lg" />
            {/if}
            <div class="flex-1">
              <div class="flex justify-between items-start mb-2">
                <div>
                  <h3 class="text-lg font-bold text-cyber-blue">{cert.name}</h3>
                  <p class="text-sm text-gray-400">{cert.issuer}</p>
                </div>
                <div class="flex gap-2">
                  <button on:click={() => handleEdit(cert)} class="text-cyber-purple hover:text-cyber-blue">✏️</button>
